# Plots of some representative examples for the variation of the number of Fourier modes
import torch
import numpy as np
...
# -----------------------
modes_list = [2, 4, 8, 16, 32, 64, 128]  #
model_paths = [
    f"../../../best_paths/best_fno_model_minmax_mode{m}_hc64_layers5_bz16_seed71.pth" for m in modes_list
]
...
# outputs
for row_idx, (m, model_path) in enumerate(zip(modes_list, model_paths), start=1):
    # model
    model = FNO(
        n_modes=(m,),
        hidden_channels=64,
    ...
    for col_idx, label in enumerate(plot_labels):
        ...
        ax.set_xlabel("t")
        if col_idx == 0:
            ax.set_ylabel(r"$\mathbf{n\_modes = " + str(m) + "}$", fontsize=14)
        else:
            ax.set_ylabel("V(t)")
        ax.grid(True)
