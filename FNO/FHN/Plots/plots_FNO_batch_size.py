# Plots of some representative examples for the variation of the batch size
import torch
import numpy as np
...
model_paths = [
    f"../../../best_paths/best_fno_model_minmax_mode32_hc64_layers5_bz{m}_seed71.pth" for m in batch_list
]
...
# outputs
for row_idx, (m, model_path) in enumerate(zip(batch_list, model_paths), start=1):
    # model
    model = FNO(
        n_modes=(32,),
        hidden_channels=64,
    ...
    for col_idx, label in enumerate(plot_labels):
        ...
        ax.set_xlabel("t")
        if col_idx == 0:
            ax.set_ylabel(r"$\mathbf{batch\_size = " + str(m) + "}$", fontsize=14)
        else:
            ax.set_ylabel("V(t)")
        ax.grid(True)
