# Plots of some representative examples for the variation of the number of hidden channels
import torch
import numpy as np
...
# -----------------------
hc_list = [8, 16, 32, 64, 128]
model_paths = [
    f"../../../best_paths/best_fno_model_minmax_mode32_hc{m}_layers5_bz16_seed71.pth" for m in hc_list
]
...
# outputs
for row_idx, (m, model_path) in enumerate(zip(hc_list, model_paths), start=1):
    # model
    model = FNO(
        n_modes=(32,),
        hidden_channels=m,
    ...
    for col_idx, label in enumerate(plot_labels):
        ...
        ax.set_xlabel("t")
        if col_idx == 0:
            ax.set_ylabel(r"$\mathbf{hidden\_channels = " + str(m) + "}$", fontsize=14)
        else:
            ax.set_ylabel("V(t)")
        ax.grid(True)
