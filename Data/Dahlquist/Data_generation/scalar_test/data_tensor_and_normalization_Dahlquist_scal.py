...
def prepare_FNO_tensors_vectorized(dataset, resolution):
    ...
    # channel
    X = amplitudes[:, None, :]  # (N,1,1)
    Y = V_values[:, None, :]  # (N,1,res)

    return torch.tensor(X, dtype=torch.float32), torch.tensor(Y, dtype=torch.float32)

# dataset loading
data_train = np.load('datasets_dict_Dahlquist_scal/dataset_train.npz', allow_pickle=True)
data_test = np.load('datasets_dict_Dahlquist_scal/dataset_test.npz', allow_pickle=True)
data_val = np.load('datasets_dict_Dahlquist_scal/dataset_val.npz', allow_pickle=True)
...
# Save .pt
folder = "data_tensors_Dahlquist_scal"
